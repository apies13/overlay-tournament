    ...
    function updateDisplay(allData) {
        for (let i = 1; i <= 4; i++) {
            ...
            if (data && data.show) {
                ...
                // Toggle upcoming highlight
                if (data.upcoming) {
                    container.classList.add('is-upcoming');
                } else {
                    container.classList.remove('is-upcoming');
                }

                // Update konten
                // Update konten
                document.getElementById(`title-${i}`).textContent = data.title || '';
                document.getElementById(`bo-${i}`).textContent = data.bo || '';
                document.getElementById(`time-display-${i}`).textContent = data.time || '';
                
                document.getElementById(`bo-${i}`).style.display = data.bo ? 'block' : 'none';
                document.getElementById(`time-display-${i}`).style.display = data.time ? 'block' : 'none';
